...
Context::Context() {
    ...
    std::vector<chaosuanDeviceType_t> device_typs;
    for (int i = 1; i < CHAOSUAN_DEVICE_TYPE_COUNT; i++) {
        device_typs.push_back(static_cast<chaosuanDeviceType_t>(i));
    }
    device_typs.push_back(CHAOSUAN_DEVICE_CPU);
    ...
    for (auto device_type : device_typs) {
        const ChaosuanRuntimeAPI *api_ = chaosuanGetRuntimeAPI(device_type);
...
void Context::setDevice(chaosuanDeviceType_t device_type, int device_id) {
