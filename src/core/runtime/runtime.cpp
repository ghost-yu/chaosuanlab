...
Runtime::Runtime(chaosuanDeviceType_t device_type, int device_id)
    : _device_type(device_type), _device_id(device_id), _is_active(false) {
    _api = chaosuan::device::getRuntimeAPI(_device_type);
...
chaosuanDeviceType_t Runtime::deviceType() const {
    return _device_type;
}
...
const ChaosuanRuntimeAPI *Runtime::api() const {
    return _api;
}
...
chaosuanStream_t Runtime::stream() const {
    return _stream;
}
