...
def load_chaosuan_model(model_path, device_name):
    model = chaosuan.models.Qwen2(model_path, chaosuan_device(device_name))
    return model
...
if __name__ == "__main__":
    ...
    model = load_chaosuan_model(model_path, args.device)
    start_time = time.time()
    chaosuan_tokens, chaosuan_output = chaosuan_infer(
        args.prompt,
        tokenizer,
        model,
        max_new_tokens=args.max_steps,
    ...
    print("\n=== Your Result ===\n")
    print("Tokens:")
    print(chaosuan_tokens)
    print("\nContents:")
    print(chaosuan_output)
    print("\n")
    print(f"Time elapsed: {(end_time - start_time):.2f}s\n")

    if args.test:
        assert chaosuan_tokens == tokens
